...
def index():
    return jsonify({
        ...
        "usage": "/dns?domain=example.com&type=A&server=8.8.8.8&key=TWOJKLUCZ"
    })
    ...
    if record_type == 'ANY':
        ...
        for rtype in RECORD_TYPES:
            try:
                ...
                pass
        if not results:
            return jsonify({"error": f"Brak rekordów dla '{domain}'"}), 404
        return jsonify({"domain": domain, "type": "ANY", "server": server, "results": results})

    try:
        records = query_dns(domain, record_type, server)
        return jsonify({"domain": domain, "type": record_type, "server": server, "results": records})
        ...
        return jsonify({"error": f"Timeout serwera {server}"}), 504
